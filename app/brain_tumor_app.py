...
def generate_hybrid_report(json_data):
    ...
    t5_input = f"generate report: {patient['name']} aged {patient['age']} has a {cls['predicted_class']} located in the **{seg['location_name']}** of the brain with confidence {cls['confidence']}. Tumor area is {seg['area_cm2']} cm2."
    ...
    if pred_class == "no_tumor":
        t5_report = (
            f"{patient['name']} aged {patient['age']} has no detectable tumor "
            f"with confidence {round(confidence, 4)}. "
            f"Tumor area is {area_cm2} cm2. Tumor size is {area_cm2} cm2."
        )
    else:
        t5_report = (
            f"{patient['name']} aged {patient['age']} has a {pred_class.replace('_', ' ')} "
            f"located in the **{location_name}** of the brain with confidence {round(confidence, 4)}. "
            f"Tumor area is {area_cm2} cm2. Tumor size is {area_cm2} cm2."
        )
